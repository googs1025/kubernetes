	...
	for _, podClaim := range pod.Spec.ResourceClaims {
		...
		if checkOwner {
			if err := resourceclaim.IsForPod(pod, claim); err != nil {
				...
				return false, fmt.Sprintf("conflicting claim needs to be removed by user: %s", err)
			}
		}
		...
		if claim.Spec.AllocationMode == resourcev1alpha2.AllocationModeWaitForFirstConsumer &&
			...
			if scheduling.Spec.SelectedNode != pod.Spec.NodeName {
				...
				return true, fmt.Sprintf("need to updated PodSchedulingContext %s/%s for scheduled pod", scheduling.Namespace, scheduling.Name)
			}
		}
		if claim.Status.Allocation != nil &&
			...
			// Need to reserve it.
			return true, fmt.Sprintf("need to reserve claim %s/%s for pod", claim.Namespace, claim.Name)
		}
	}
...
func (ec *Controller) ensurePodSchedulingContext(ctx context.Context, pod *v1.Pod) error {
	...
	if scheduling == nil {
		...
		if _, err := ec.kubeClient.ResourceV1alpha2().PodSchedulingContexts(pod.Namespace).Create(ctx, scheduling, metav1.CreateOptions{}); err != nil {
			return fmt.Errorf("create PodSchedulingContext %s/%s: %v", scheduling.Namespace, scheduling.Name, err)
		}
		return nil
	}
	...
	if scheduling.Spec.SelectedNode != pod.Spec.NodeName {
		...
		if _, err := ec.kubeClient.ResourceV1alpha2().PodSchedulingContexts(pod.Namespace).Update(ctx, scheduling, metav1.UpdateOptions{}); err != nil {
			return fmt.Errorf("update spec.selectedNode in PodSchedulingContext %s/%s: %v", scheduling.Namespace, scheduling.Name, err)
		}
	}
...
func (ec *Controller) reserveForPod(ctx context.Context, pod *v1.Pod, claim *resourcev1alpha2.ResourceClaim) error {
	...
	if _, err := ec.kubeClient.ResourceV1alpha2().ResourceClaims(claim.Namespace).UpdateStatus(ctx, claim, metav1.UpdateOptions{}); err != nil {
		return fmt.Errorf("reserve claim %s/%s for pod: %v", claim.Namespace, claim.Name, err)
	}
	return nil
}
		...
		if podName != "" {
			...
			case err == nil:
				...
				if pod.UID != podUID || isPodDone(pod) {
					...
					if err != nil {
						return fmt.Errorf("delete claim %s/%s: %v", claim.Namespace, claim.Name, err)
					}
				} else {
